\secrel{\al}\label{L}\secdown

\secrel{Общая информация}\secdown
...
раздел описывает L-компонент платформы ALYEH: \al.
...
названием \prog{BuildRoot}\ref{buildroot}.

\secrel{Требования к системе сборки (\var{BUILD}-хост)}
...
\lst{добавить в \file{/etc/fstab}}{}{L/doc/fstab.txt}
...
\cmd{make gcc ramclean}, а потом примонтировать \file{tmpfs}\
опять на \file{src}.
...
Прежде чем продолжить, введем понятие \termdef{пакет}{пакет}. В \L\
...
\lstx{Makefile}{}{tmp/L_Makefile.mk}{mk}\index{L!Makefile}

\secrel{Клонирование проекта \pack{\al}} \label{azclone}
...
\begin{itemize}[nosep]
  ...
  каталогов, которые нужно зачистить; по умолчанию это \cmd{rm -rf}, заданием
  \cmd{echo}\ зачистка отключается (нужно если вы собираете систему в
...
\begin{enumerate}[nosep]
  ...
  \end{enumerate}
  \item \pack{libs} сборка библиотек \var{\$\{LIBS\}} \ref{azlibs}
  \item \pack{apps} сборка прикладных пакетов \var{\$\{APPS\}} \ref{azapps}
...
\lstx{mk/dirs.mk}{}{L/mk/dirs.mk}{mk}\index{L!dirs}

Встроенная переменная \var{PWD}\ содержит полное имя каталога, из которого был
запущен \pack{make}.

\bigskip
Каталог зеркала архивов исхоных текстов программ

\lstx{GZ}{}{tmp/L_GZ.mk}{mk}\index{L!GZ}

Каталог распаковки исходных текстов: некоторые пакеты должна собираться в дереве
исходников

\lstx{SRC}{}{tmp/L_SRC.mk}{mk}\index{L!SRC}
...
\lstx{TMP}{}{tmp/L_TMP.mk}{mk}\index{L!TMP}

Переменная \var{BUILD}\ задает \term{триплет}\ системы, на которой вы
...
загрузчика. Имя \var{ROOT}\ создается из двух переменных, описанных далее: имя
аппаратной платформы \var{HW}\ref{azhw}\ и имени приложения
\var{APP}\ref{azapp}.

\lstx{ROOT}{}{tmp/L_ROOT.mk}{mk}\index{L!ROOT}
\lstx{BOOT}{}{tmp/L_BOOT.mk}{mk}\index{L!BOOT}

Список всех рабочих каталогов в одной переменной:

\lstx{DIRS}{}{tmp/L_DIRS.mk}{mk}\index{L!DIRS}

\secrel{\pack{gz}: Загрузка архивов исходников} \label{azgz}

\lstx{mk/gz.mk}{}{L/mk/gz.mk}{mk}

\secrel{\var{APP}: Приложение}\label{azapp}

\term{Приложение}\ --- короткое кодовое название вашего варианта сборки системы
в целом.

Приложение задается в файле \file{mk/head.mk}\ через переменную \var{APP},
доступные значения:
...
\begin{enumerate}
  ...
  \item \pack{math}: система кластерных вычислений
\end{enumerate}

Значение переменной \var{APP}\ по умолчанию задано в \file{mk/head.mk}:

\lstx{\var{APP} @ \file{hw/head.mk}}{}{tmp/L_APP.mk}{mk}
\index{L!настройка!приложение}
\index{L!переменная!APP}
...
\begin{itemize}
  \item \var{LIBS}: набор используемых библиотек
  \item \var{PACKS}: набор используемых программных пакетов \ref{azpacks}
\end{itemize}

\lstx{app/micro.mk}{}{L/app/micro.mk}{mk}\index{L!приложение!micro}

\lstx{app/clock.mk}{}{L/app/clock.mk}{mk}\index{L!приложение!clock}

\secup

\secrel{\var{HW}: Поддерживаемое железо}\label{azhw}\index{L!железо}

Конфигурация целевого железа задается в файле \file{mk/head.mk}\ через
переменную \var{HW}, доступные значения приведены в таблице:
...
\lstx{mk/head.mk}{}{L/mk/head.mk}{mk}

\secdown

\secrel{i386}\secdown\index{L!железо!i386}
...
% \lstx{hw/qemu386.mk}{}{../azlin/hw/qemu386.mk}{mk}\index{L!железо!QEMU}
...
% \bigskip
% 
% \lstx{hw/eeepc701.mk}{}{../azlin/hw/eeepc701.mk}{mk}\index{L!железо!ASUS Eee PC 701}
...
% \bigskip
% 
% \lstx{hw/gac1037.mk}{}{../azlin/hw/gac1037.mk}{mk}\index{L!железо!Gigabyte GA-C1037UN-EU}
...
% \lstx{hw/qemuARM.mk}{}{../azlin/hw/qemuARM.mk}{mk}\index{L!железо!QEMU}
...
% \lstx{hw/qemuMIPS.mk}{}{../azlin/hw/qemuMIPS.mk}{mk}\index{L!железо!QEMU}
...
\clearpage
\secrel{vocore: VoCore}\label{vocore} \index{L!железо!VoCore}
...
\secrel{bswift: BlackSwift} \index{L!железо!BlackSwift}

\href{http://habrahabr.ru/post/242731/}{bswift}

\secup\secup

\secrel{\var{CPU}: Конфигурации процессоров}\secdown

% Настройки на процессор задаются в файле \file{cpu/}\var{\$\{CPU\}}\file{.mk}.
% 
% \begin{tabular}{p{0.15\textwidth} p{0.8\textwidth}}
% \var{ARCH} & архитектура целевой системы, используется при конфигурировании
% ядра \\
% \var{TARGET} & \term{триплет целевой системы}, параметр задает тип целевой
% системы при сборке кросс-компилятора и используется во всех скриптах
% \pack{configure}\ при сборке остальных пакетов \\
% \var{CFG\_CPU} & параметры при сборке кросс-компилятора \\
% \var{CPU\_FLAGS} & параметры \prog{gcc}\ для оптимизации кода \\
% \end{tabular}

\secrel{i386}

% \lstx{cpu/i486sx.mk}{}{azlin/cpu/i486sx.mk}{mk}
% \index{L!железо!i486sx}
% 
% \lstx{cpu/CeleronM.mk}{}{azlin/cpu/CeleronM.mk}{mk}
% \index{L!железо!CeleronM}
% 
% \lstx{cpu/Celeron1037U.mk}{}{azlin/cpu/Celeron1037U.mk}{mk}
% \index{L!железо!Celeron1037U}

\secrel{ARM}

\secrel{MIPS}

% \lstx{cpu/AR7240.mk}{}{../azlin/cpu/AR7240.mk}{mk}\index{L!железо!AR7240}
% 
% \lstx{cpu/RT5350.mk}{}{../azlin/cpu/RT5350.mk}{mk}\index{L!железо!RT5350}
...
\secrel{\pack{cross0}: сборка минимального кросс-компилятора}\label{azcross0}
\secdown

% \lstx{mk/versions.mk}{}{tmp/azlin_versions.cross}{mk}
% \lstx{}{}{azlin/mk/cross.mk}{mk}

\secrel{\pack{cclibs0}: библиотеки для сборки \pack{gcc}} \label{azcclibs}

% \pack{gmp} \pack{mpfr} \pack{mpc} \pack{isl} \pack{cloog}

\secrel{\pack{binutils0}: ассемблер, линкер и утилиты} \label{azbinutils0}

% \begin{tabular}{l l}
% --target=\$(\var{TARGET})& триплет целевой платформы\\
% \$(\var{CFG\_ARCH})&параметры архитектуры\\
% \$(\var{CFG\_CPU})&параметры процессора\\
% --program-prefix&префикс \verb|<prefix>-(as|ld|..)|\\
% --with-sysroot& каталог в котором находятся хедеры и библиотеки\\
% --with-native-system-header-dir=\file{/include}&каталог с хедерами\\
...
\secrel{\pack{gcc0}: сборка минимального кросс-компилятора Си}\label{azgcc0}
...
\secrel{\pack{core}: сборка основной системы} \label{azcore}\secdown
...
%   \item сборка ядра
%   \item копирование готового файла ядра в \var{\$\{BOOT\}}
%   \item генерация \term{хедеров}\ в \var{\$\{ROOT\}}
% \end{enumerate}
% 
% \bigskip
% Все параметры ядра идут с префиксом \var{CONFIG\_}:
% 
% \begin{itemize}[nosep]
%   \item \pack{core.mk}
%   \begin{itemize}[nosep]
%     \item \var{CROSS\_COMPILE} префикс кросс-компилятора
%     \verb#<pfx>-(gcc|as|ld..)#
%     \item \var{LOCALVERSION} суффикс ядра \verb|linux-x.x.x-suffix|
%     \item \var{DEFAULT\_HOSTNAME} имя компьютера \file{<host>.<domain>}
%   \end{itemize}
%   \item \pack{all} \emph{для всех платформ}
%   \begin{itemize}[nosep]
%   \item \var{NOHIGHMEM=y} сборка в режиме $\leq 1$\,Gb \ram
%   \item \var{KERNEL\_GZIP=y} ядро сжимать алгоритмом \pack{gzip}\note{важно для
%   загрузчиков на не-i386 системах}
%   \item \emph{корневая файловая система в initrd/ramdisk}
%   \begin{itemize}[nosep]
%   \item \var{BLK\_DEV\_INITRD=y} rootfs в \ram (initrd)
%   \item \var{PROC\_FS=y} файловая подсистема \file{/proc}
%   \item \var{SYSFS=y} файловая подсистема \file{/sys}
%   \item \var{DEVTMPFS=y} файловая подсистема \file{/dev} (интерфейсы драйверов
%   устройств)
%   \item \var{DEVTMPFS\_MOUNT=y} автоматически монтировать \pack{devfs}
%   \end{itemize}
% 
%   \item \emph{режим реального времени\note{повышенная или
%   \emph{гарантированная} отзывчивость системы на внешние события}} \ref{linrt}
%   \begin{itemize}[nosep]
%   \item \var{PREEMPT=y} \term{вытесняющая многозадачность} в режиме ядра
%   \item \var{HZ\_1000=y} системный таймер 1\,KHz
%   \end{itemize}
% 
%   \item \emph{исполняемые форматы файлов}
%   \begin{itemize}[nosep]
%   \item \var{BINFMT\_ELF=y} бинарные файлы в формате \pack{ELF}
%   \item \var{BINFMT\_SCRIPT=y} файлы скриптов с \verb|#!/bin/sh| в заголовке
%   \item \var{COREDUMP=n} не писать \term{корки} при сбоях
%   \end{itemize}
% 
%   \item \emph{поддержка консоли tty}
%   \begin{itemize}[nosep]
%   \item \var{TTY=y} последовательные порты и командная консоль
%   \item \var{UNIX98\_PTYS=n} псевдотерминалы UNIX98
%   \item \var{LEGACY\_PTYS=n} псевдотерминалы UNIX/BSD
%   \end{itemize}
% 
%   \item \emph{клавиатура}
%   \begin{itemize}[nosep]
%   \item \var{INPUT\_KEYBOARD=y} ввод с аппаратной клавиатуры
%   \end{itemize}
% 
%   \item \emph{мышь}
%   \begin{itemize}[nosep]
%   \item \var{INPUT\_MOUSE=y}
%   \item \var{INPUT\_MOUSEDEV=y}
%   \item \var{INPUT\_MOUSEDEV\_PSAUX=n} не создавать \verb|/dev/psaux|
%   \end{itemize}
% 
...
%   \begin{itemize}[nosep]
%   \item \var{HID=y} [H]uman [I]nterface [D]evice
%   \item \var{HID\_GENERIC=y} универсальный драйвер USB HID Class
%   \end{itemize}
% 
%   \item \emph{Графический режим \term{FrameBuffer}}
%   \begin{itemize}[nosep]
%   \item \var{FB=y}
%   \item \var{FRAMEBUFFER\_CONSOLE=y} командная консоль
%   \item \var{LOGO=y} вывод пингвина при запуске
%   \item \var{LOGO\_LINUX\_MONO=y} выводить только черно-белый вариант
%   \item \var{LOGO\_LINUX\_VGA16=n}
%   \item \var{LOGO\_LINUX\_CLUT224=n}
%   \end{itemize}
% 
%   \item \emph{Отладка и \term{printk} лог ядра}
%   \begin{itemize}[nosep]
%   \item \var{DEBUG\_KERNEL=y} включение отладочных функций ядра
%   \item \var{EARLY\_PRINTK=y} вывод пусковой части ядра
%   \item \var{PRINTK=y} вывод главного ситемного лога ядра \term{printk}
%   \item \var{PRINTK\_TIME=y} выводить метки времени (для отладки времени
%   запуска)
%   \item \var{SCHED\_DEBUG=n} не отлаживать планировщик
%   \item \var{DEBUG\_PREEMPT=n} не отлаживать вытесняющий режим
%   \end{itemize}
% 
%   \end{itemize}
...
%   \begin{itemize}
% 
% \item \var{X86\_GENERIC=y} универсальная оптимизация для всех i386-процессоров
% 
% \item \pack{UART/COM/RS232} \emph{последоваельные порты}
%   \begin{itemize}[nosep]
% \item \var{SERIAL\_8250=y} UART 8250/16550
% \item \var{SERIAL\_8250\_CONSOLE=y} командандная консоль на COM-портах
%   \end{itemize}
% 
% \item \var{VGA\_CONSOLE=y} командандная консоль на VGA 80$\times$25
% 
% \item  \emph{FrameBuffer}
%   \begin{itemize}[nosep]
% \item \var{FB\_VESA=y} универсальный видеодрайвер VESA 2.0+
%   \end{itemize}
% 
% \item  \emph{клава}
%   \begin{itemize}[nosep]
% \item \var{KEYBOARD\_ATKBD=y} клавиатура PC AT / PS2
%   \end{itemize}
% 
% \item  \emph{мышь}
%   \begin{itemize}[nosep]
% \item \var{MOUSE\_PS2=y} мышь PS/2
% \item \var{MOUSE\_SERIAL=y} (старая) мышь на RS232
%   \end{itemize}
% 
% \item \var{NVRAM=y} доступ к памяти CMOS
% 
% \item \emph{отладка}
%   \begin{itemize}[nosep]
% \item \var{MAGIC\_SYSRQ=y} волшебная кнопка \keys{Alt+SysRq}
% \item \var{X86\_VERBOSE\_BOOTUP=y} доп.сообщения при пуске ядра
%   \end{itemize}
% 
%   \end{itemize}
% 
...
% \begin{itemize}[nosep]
%   \item \var{M486=y}
%   \item \var{MATH\_EMULATION=y} включить программную эмуляцию мат.сопроцессора
%   (FPU) в ядре
% \end{itemize}
% 
...
% 
% \begin{itemize}[nosep]
% \item \var{MATH\_EMULATION=n} выключить эмуляцию FPU: \pack{QEMU} запускается
% на процессорах Pentium и старше, которые всегда имеют аппаратный модуль
% плавающей точки
% \item \var{HZ\_1000=n}  в режиме эмуляции быстрый таймер не
% имеет смысла,
% 
% \item \var{HZ\_100=y} переключаемся на 100\,Hz
% \end{itemize}
% 
% \item \pack{app} \emph{micro}
% 
% \begin{itemize}[nosep]
% \item \var{FB=n} выключаем поддерку FrameBuffer,..
% \item \var{INPUT\_MOUSE=n} мыши
% \end{itemize}
% 
% \end{itemize}
...
%   \item инсталляция в \var{ROOT}
%   \item инсталляция утилит для работы с shared библиотеками \emph{на целевой
%   системе}
%   \item инсталляция утилит для \emph{\var{BUILD}-системы}
...
% \item
% \begin{itemize}[nosep]
% \item \var{ARCH\_HAS\_MMU=y} \emph{всегда включено}
% \item \var{ARCH\_USE\_MMU=y} \emph{всегда включено}
% \item \var{UCLIBC\_HAS\_FPU=y} \emph{всегда включено}: используется эмулятор в
% ядре
% \item \var{UCLIBC\_HAS\_FLOATS=y} функции плавающей точки
% \item \var{DO\_C99\_MATH=n} функции float math C99
% \item \var{UCLIBC\_CTOR\_DTOR=y} конструктор/декструктор
% \item \var{UCLIBC\_HAS\_LFS=y} большие файлы
% \end{itemize}
% \item \emph{разделяемые \term{shared} библиотеки}
% \begin{itemize}[nosep]
%   \item \var{HAVE\_SHARED=y}
%   \item \var{LDSO\_CACHE\_SUPPORT=y} \file{/etc/ld.co.conf}
%   \item \var{LDSO\_PRELOAD\_ENV\_SUPPORT=n} переменная \var{LD\_PRELOAD}
%   содержит список библиотек, загружаемых первыми
%   \item \var{LDSO\_LD\_LIBRARY\_PATH=n} отключить
%   переменную \var{LD\_LIBRARY\_PATH} со списком каталогов поиска shared
%   библиотек
% \end{itemize}
% \item \emph{многопоточные \term{threads} программы }
% \begin{itemize}[nosep]
%   \item \var{LINUXTHREADS\_OLD=y} \emph{старый} вариант потоков, новый вызывает
%   segfault при использовании ключей gcc -lpthread и использовании shared ulibc
% \end{itemize}
% \item \emph{параметры установки}
% \begin{itemize}[nosep]
% \item \var{DOSTRIP=y}
% \item \var{RUNTIME\_PREFIX=""}
% \item \var{DEVEL\_PREFIX=""}
% \end{itemize}
% \item \emph{опции необходимые \pack{busybox}}
% \begin{itemize}[nosep]
% \item \var{UCLIBC\_HAS\_CTYPE\_TABLES=y}
% \item \var{UCLIBC\_HAS\_NETWORK\_SUPPORT=y}
% \item \var{UCLIBC\_HAS\_FNMATCH=y}
% \item \var{UCLIBC\_HAS\_GNU\_GETOPT=y}
% \item \var{UCLIBC\_HAS\_REGEX=y}
% \item \var{UCLIBC\_SUSV3\_LEGACY=y}
% \end{itemize}
% \end{itemize}
% 
...
\secrel{\pack{gcc}: пересборка полного кросс-компилятора Си/\cpp}\label{azgcc}

После сборки \pack{ulibc}\ необходимо еще раз пересобрать \pack{gcc}\ с полными
настройками. Пакет собирается \emph{после сборки \pack{core}}.

\secrel{\pack{busybox}: набор базовых утилит UNIX} \label{azbb}
...
%   \file{app/\$\{\var{APP}\}.bb}}
%   \item сборка и инсталляция в \var{ROOT}
% \end{enumerate}
% 
% \bigskip
...
\secrel{\pack{libs}: сборка библиотек \var{\$\{LIBS\}}} \label{azlibs}

% \lstx{mk/versions.mk}{}{tmp/azlin_versions.libs}{mk}

\secrel{\pack{apps}: сборка прикладных пакетов \var{\$\{APPS\}}}\label{azapps}
...
%   выражением} в переменной \var{ROOTREX}
% \end{enumerate}
% 
...
\secrel{\pack{boot}: сборка загрузчика}\label{azboot}
\secdown

% \lstx{mk/versions.mk}{}{tmp/azlin_versions.boot}{mk}
% \lstx{mk/boot.mk}{}{azlin/mk/boot.mk}{mk}

\secrel{\pack{syslinux} /i386/}

\secrel{\pack{grub} /i386/}

\secrel{\pack{uboot}: загрузчик u-boot}
...
\secrel{\pack{pxeboot} /i386/}

\secrel{сетевая загрузка через u-boot}
...
% 
% Переменная \var{QEMU\_CFG}\ задает параметры запуска \prog{QEMU}:
% 
% \begin{tabular}{l l}
...
\secrel{math: компьютерная математика}\secdown

\secrel{\pack{blas}: библиотека \prog{BLAS}}\label{azblas}

\secrel{\pack{lapack}: библиотека \prog{LAPACK}}\label{azlapack}

\secrel{\pack{octave}: cистема численных методов \prog{Octave}}\label{azoctave}

% \secrel{\pack{maxima}: система аналитической математики
...
% численных)} (GCL \ref{azgcl}/\prog{Maxima}\ref{azmaxima}),
%   \item
...
%   кросс-компилятор, т.е. \var{TARGET}\ нашей встраиваемой системы:
