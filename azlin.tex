...
\subsection{\prog{dirs}: создание дерева каталогов}

\subsection{\prog{gz}: закачка архивов исходников}

\subsection{\prog{tc}: сборка кросс-компилятора}

\subsubsection{\prog{binutils}: ассемблер, линкер и утилиты}

\subsubsection{\prog{cclibs}: \prog{gmp} \prog{mpfr} \prog{mpc}}

\subsubsection{\prog{gcc0}: сборка минимального кросс-компилятора Си}

\subsubsection{\prog{gcc}: сборка кросс-компилятора Си/\cpp}

Пакет собирается \emph{после сборки \prog{core}}. 

\subsection{\prog{core}: сборка основной системы}

\subsubsection{\prog{kernel}: ядро \linux}

\subsubsection{\prog{uclibc}: библиотека \prog{uClibc}}

\subsubsection{\prog{bb}: набор утилит \prog{busybox}}

\subsection{\prog{libs}: сборка библиотек}

\subsection{\prog{apps}: сборка прикладных пакетов}

\subsection{\prog{user}: сборка пользовательского кода}

\subsection{\prog{root}: формирование корневой файловой системы}
\subsection{\prog{boot}: сборка загрузчика}

\subsection{\prog{emu}: запуск собранной системы в эмуляторе}
