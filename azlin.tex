...
\secrel{CPU: Конфигурации процессоров}
...
\secrel{Пакеты}\label{azpacks}

\subsection{\file{versions.mk}: Версии пакетов}\label{azpackver}

\subsubsection{\pack{tc}: кросс-компилятор}

\lstx{mk/versions.mk}{}{azlin/doc/versions.cross}{mk}

\subsubsection{\pack{core}: ядро}

\lstx{mk/versions.mk}{}{azlin/doc/versions.core}{mk}

\subsubsection{\pack{boot}: загрузчики}

\lstx{mk/versions.mk}{}{azlin/doc/versions.boot}{mk}

\subsubsection{\pack{libs}: библиотеки}

\lstx{mk/versions.mk}{}{azlin/doc/versions.libs}{mk}

\subsection{\pack{tc}: сборка кросс-компилятора} \label{aztc}

\lstx{}{}{azlin/mk/cross.mk}{mk}

\subsubsection{\pack{binutils}: ассемблер, линкер и утилиты} \label{azbinutils}
...
\subsubsection{\pack{cclibs}: библиотеки для сборки \pack{gcc}} \label{azcclibs}

\pack{gmp} \pack{mpfr} \pack{mpc}

\subsubsection{\pack{gcc0}: сборка минимального кросс-компилятора Си}
\label{azgcc0}
...
\subsubsection{\pack{gcc}: пересборка полного кросс-компилятора Си/\cpp}
\label{azgcc}

Пакет собирается \emph{после сборки \pack{core}}. 

\subsection{\pack{core}: сборка основной системы} \label{azcore}

\lstx{mk/core.mk}{}{azlin/mk/core.mk}{mk}

\subsubsection{\pack{kernel}: ядро \linux} \label{azkernel}
...
\subsubsection{\pack{ulibc}: библиотека \pack{uClibc}} \label{azulibc}
...
\subsubsection{\pack{gcc}: пересборка полного \pack{gcc}} \label{azgcc}

После сборки \pack{ulibc}\ необходимо еще раз пересобрать \pack{gcc}\ с полными
настройками.

\subsubsection{\pack{busybox}: набор утилит \pack{busybox}} \label{azbb}
...
\subsection{\pack{libs}: сборка библиотек \file{\$\{LIBS\}}} \label{azlibs}

\subsection{\pack{apps}: сборка прикладных пакетов \file{\$\{APPS\}}}
\label{azapps}

\subsection{\pack{user}: сборка пользовательского кода} \label{azuser}

\subsection{\pack{root}: формирование корневой файловой системы} \label{azroot}
...
\subsection{\pack{boot}: сборка загрузчика
\pack{syslinux}/\pack{grub}/\pack{uboot}} \label{azboot}

\lstx{mk/boot.mk}{}{azlin/mk/boot.mk}{mk}

\subsubsection{\pack{syslinux}}

\subsubsection{\pack{grub}}

\subsubsection{\pack{uboot}}
