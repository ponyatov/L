...
\subsection{\file{versions.mk}: Версии пакетов}\label{azpackver}

\subsubsection{\prog{tc}: кросс-компилятор}

\lstx{mk/versions.mk}{}{azlin/doc/versions.cross}{mk}

\subsubsection{\prog{core}: ядро}

\lstx{mk/versions.mk}{}{azlin/doc/versions.core}{mk}

\subsubsection{\prog{boot}: загрузчики}

\lstx{mk/versions.mk}{}{azlin/doc/versions.boot}{mk}

\subsubsection{\prog{libs}: библиотеки}
...
При сборке \prog{gcc0}/\prog{gcc} отключайте \prog{ccache}: кэш при разовых сборках
...
\bigskip
Особенность \prog{busybox}\ --- скрипты конфигурации не умеют отрабатывать
...
\begin{enumerate}
  ...
  \item \file{/share/}
  \item создание \prog{initrd} используя фильтрацию файлов \term{регулярным
...
\begin{enumerate}
  ...
  \item запуск \term{демона} \prog{mdev}, обслуживающего \file{devfs}:
...
\prog{syslinux}/\prog{grub}/\prog{uboot}} \label{azboot}
